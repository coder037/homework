...
public class RuntimeDefaults {
	...
	int countOfCountries = 1; // need to decr 1 later
	String defaultCountry = "EE"; // Which is Estonia, of course
	int timeoutBetweenDownloadsMin = 3; // seconds
	int timeoutWhileWorkingMax = 1800; // seconds, some unclean bailout after
										// that
	boolean interactive = false; // asking all these questions on CLI -> -y/N
	boolean extraSources = false; // probably no time to implement extra sources
									// like CleanMX
	boolean outputDefinedManually = false; // so far not said anything on the
											// command line
